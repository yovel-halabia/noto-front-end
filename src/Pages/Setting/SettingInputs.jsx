...
export default function SettingInputs() {
	...
	useEffect(() => {
		inputDispatch({
			type: "updateState",
			payload: { fullName: { ...inputState.fullName, val: data?.fullName } },
		});
		inputDispatch({
			type: "updateState",
			payload: { email: { ...inputState.email, val: data?.email } },
		});
	}, [data]);
					...
					Object.keys(data.inputsError).forEach((key) => {
						inputDispatch({
							...
							payload: {
								[key]: {
									...inputState[key],
									errText: data.inputsError[key].message,
									trhowErr: true,
								},
							},
						});
	...
	return (
		<>
			<div className="setting-inputs__container">
				<Input
					...
					onChange={(e) =>
						inputDispatch({
							type: "updateState",
							payload: { fullName: { ...inputState.fullName, val: e.target.value } },
						})
					}
				...
				<Input
					...
					onChange={(e) =>
						inputDispatch({
							type: "updateState",
							payload: { email: { ...inputState.email, val: e.target.value } },
						})
					}
				...
				<div className="setting-inputs__password-container">
					<Input
						...
						onChange={(e) =>
							inputDispatch({
								type: "updateState",
								payload: { password: { ...inputState.passwrod, val: e.target.value } },
							})
						}
			...
			{inputState.showIndication === "data" && (
				<ErrAlert text="something went wrong try agin later" />
			)}
			{inputState.showIndication === "success" && (
				<SuccessAlert text="Changes saved successfully" />
			)}
		</>
	);
}
